...
# # Run 5 containers at the same time
# executor = ThreadPoolExecutor(max_workers=5)


# def run_code_task(language, code):
#     file_id = uuid.uuid4().hex
#     filename = f"temp_{file_id}"
...
#     if language == "python":
#         cmd += ["python:3.11-slim", "python", f"/app/{filename}"]
#     elif language == "javascript":
#         cmd += ["node:18-slim", "node", f"/app/{filename}"]
...
#     # submit to thread pool → runs in background thread
#     future = executor.submit(run_code_task, language, code)

#     result = future.result()  # wait for only this task
